...
#        self.dbClient = dbClient
    ...
    def tic(self,software_timeout=False):
        ...
        try:
            ...
            for t in self.tires:
                self.logger.info(f"tire {t.name},{type(t)} tic")
                try:
                    t.tic(stable=s,anfang=a,ende=e,interval=self.gear.interval,gearR=self.gear.r,HA=self.gear.isHA)
                except Exception as exx:
                    print('****',exx)
                    traceback.print_exc()
...
class rawWriter(QtCore.QObject):
    def __init__(self,config):
        ...
        port = int(config[ 'port'])
        self.influx_online = False
        dbClient = InfluxDBClient(host=host, port=port, database=database,timeout=0.1,retries=1)
        self.dbClient=dbClient
...
#        self.ticinterval=self.config.getint('general','interval')
    ...
    def writeRawValue(self,data):
        ...
        #js = [{"time": datetime.utcnow(),
        js = [{"time":  datetime.now(timezone.utc),
              "measurement": "tireslip_raw",
              "tags": {
                  "id": channel,
              },
        ...
        try:
            if self.influx_online:
                r=self.dbClient.write_points(js)
        except Exception as e:
            print(e)
...
class tire(object):
    def __init__(self,config,influxOnline,dbClient,name,parent=None):
        ...
        self.mininterval = float(self.config["mininterval"])*1000000
        try:
            self.mininterval_HA = float(self.config["mininterval_HA"])*1000000
        except:
            self.mininterval_HA = self.mininterval
    ...
    def tic(self,stable,anfang,ende,interval,gearR=0,HA=False):
        f=1
#        if gearR>0:
#            f=gearR/self.r
#            interval=interval/f
        f=self.r2/self.r
        interval=interval/f
        self.logger.info(f'Hilfsantrieb {HA}')

        if not stable:
            self.logger.info('not stable')
            self.error=1
            self.status=0
            return
        data=None
        if self.rising:
            data_rising=self.selectData(anfang,ende,interval,rising=True,HA=HA)
            data=data_rising
        if self.falling:
            data_falling=self.selectData(anfang,ende,interval,rising=False,HA=HA)
            if data is None:
                data=data_falling
                print("no data")
            else:
    ...
    def selectData(self,  anfang, ende, interval,rising=True,HA=False):
        if anfang> ende:
            return  None
        if rising:
            edge = "rising"
...
#        data = data[data["timestamp"].diff() > self.mininterval]
        if not HA:
            ...
            data = data[(data["timestamp"].diff() > self.mininterval_HA) | (data["timestamp"].diff() < 0)]
...
class gear(object):
    def __init__(self,config,influxOnline,influxClient):
        ...
        self.HA=False
        if 'sigma_HA' in self.config:
            self.HA=True
            self.minRPM_HA=float(self.config["minRPM_HA"])
            self.maxRPM_HA=float(self.config["maxRPM_HA"])
            self.sigma_HA=float(self.config["sigma_HA"])
