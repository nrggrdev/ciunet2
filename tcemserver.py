#------------------------------------------------------------------------------------------
__version__ = "v1.0 jh201106a Python 3.9 64Bit"
#===---------------------------------------------------------------------------------------
import sys
import time
import socket
...
def receive_files():
    # mystruct = struct.Struct("!28sI")
    # FILEINFO_SIZE = mystruct.size
    SEPARATOR = "<SEPARATOR>"
    ...
    while 1:
        try:
            ...
            l=len(received)
            i=int(received)
            logging.info(f'ultralänge: {l}, {received},{i}')
            
            received = client_socket.recv(i)
            l=len(received)
            logging.info(f"size filedescriptor:{l}")
            try:
            ...
            with open(filename, 'wb') as f:
                while True:
                    ...
                    bytes_read = client_socket.recv(int(filesize))
                    if not bytes_read:
                        break
                    f.write(bytes_read)
            client_socket.close()
